"""
Complexity-Aware Response Cache
...
from src.agents.constants import CACHE_CLEANUP_THRESHOLD
...
class ComplexityAwareCache:
    ...
    async def get_or_execute_supervisor_plan(
        self,
        query: str,
        execute_fn: Callable,
        user_id: Optional[int] = None
        ...
        """
        Cache Supervisor execution plans (High Complexity).
        TTL: 5 minutes (plans can change with context).
        
        Args:
            query: User query
            execute_fn: Async function generating plan
            user_id: Optional user ID
            
        Returns:
            Execution plan list
        """
        return await self.get_or_execute_async(
            query=query,
            execute_fn=execute_fn,
            complexity_level="high",  # 5 min TTL
            user_id=user_id
        )

    async def get_or_execute_routing(
        self,
        query: str,
        execute_fn: Callable,
        user_id: Optional[int] = None
        ...
        """
        Cache Supervisor routing decisions (Medium Complexity).
        TTL: 15 minutes (routing is relatively stable).
        
        Args:
            query: User query
            execute_fn: Async function deciding route
            user_id: Optional user ID
        ...
        return await self.get_or_execute_async(
            query=query,
            execute_fn=execute_fn,
            complexity_level="medium",  # 15 min TTL
            user_id=user_id
        )
