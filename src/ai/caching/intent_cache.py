"""
Request-Level Intent Patterns Cache
...
from src.agents.constants import MAX_INTERACTION_HISTORY
...
class IntentPatternsCache:
    ...
    def get_supervisor_plan(self, query: str) -> Optional[list]:
        """
        Get cached supervisor execution plan or return None if not cached.
        
        Args:
            query: User query
            
        Returns:
            Cached supervisor plan or None
        """
        key = f"supervisor_plan:{query}"
        if key in self._cache:
            self._hit_count += 1
            return self._cache[key]
    ...
    def set_supervisor_plan(self, query: str, plan: list):
        """
        Cache supervisor execution plan.
    ...
    def get_routing_decision(self, query: str) -> Optional[str]:
        """
        Get cached routing decision (agent name) or return None if not cached.
        
        Args:
            query: User query
            
        Returns:
            Cached agent name or None
        """
        key = f"routing:{query}"
        if key in self._cache:
            self._hit_count += 1
    ...
    def set_routing_decision(self, query: str, agent_name: str):
        """
