...
if HAS_INTENT_PATTERNS:
    ...
    ]))[:15]  # Increased from 10 to 15 for better coverage
else:
    TASK_KEYWORDS = []
...
NOTION_KEYWORDS = list(set([
    ...
    for kw in pattern.split()
]))[:15] if HAS_NOTION_PATTERNS else []

CREATION_VERBS = ACTION_VERBS[:15]  # Increased from 10 to 15
...
def has_notion_keywords(query: str) -> bool:
    ...
    """
    if not HAS_NOTION_PATTERNS or not NOTION_KEYWORDS:
...
def is_calendar_creation_query(query: str) -> bool:
    ...
    # Day of week references (Monday, Tuesday, etc.)
...
def is_notion_creation_query(query: str) -> bool:
    ...
    """
    return _is_creation_query(query, NOTION_KEYWORDS if HAS_NOTION_PATTERNS else ['notion', 'page', 'database'])
...
def get_query_domain(query: str) -> str:
    ...
    notion_count = sum(1 for keyword in (NOTION_KEYWORDS if HAS_NOTION_PATTERNS else ['notion', 'page', 'database']) 
...
def is_multi_domain_query(query: str) -> bool:
    ...
    return len(domains) > 1 or (len(domains) == 1 and domains[0] != 'general')
